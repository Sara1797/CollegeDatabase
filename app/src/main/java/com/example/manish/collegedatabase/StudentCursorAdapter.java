...
public class StudentCursorAdapter extends CursorAdapter {
    ...
    public void bindView(View view, Context context, Cursor cursor) {
        ...
        int nameColumnIndex = cursor.getColumnIndex(StudentContract.StudentEntry.COLUMN_STUDENT_NAME);


        // Read the pet attributes from the Cursor for the current pet
        String studentName = cursor.getString(nameColumnIndex);
        int studentId = cursor.getInt(cursor.getColumnIndexOrThrow(StudentContract.StudentEntry.COLUMN_STUDENT_ID));


        // Update the TextViews with the attributes for the current pet
        nameTextView.setText(studentName);
        summaryTextView.setText(String.valueOf(studentId));
    }
}